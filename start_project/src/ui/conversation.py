...
class Conversation:
    def __init__(self, font):
        ...
        self.dialogs = [
            'Hello world',
            'Lorem Ipsum 123456798765434567',
            'word word word \ntest test test',
        ]
        self.active_dialog = 0
        self.message_done = False
        self.finish_conversation = False
        self.speed = 40
    ...
    def draw(self, display, dt, current_time):
        if not self.finish_conversation:
        ...
        if action:
            if self.message_done:
                ...
                if self.active_dialog >= len(self.dialogs):
                    # self.active_dialog = 0 # loop
                    self.finish_conversation = True # end
